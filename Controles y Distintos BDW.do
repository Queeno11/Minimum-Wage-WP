...
rdrobust e`j' z, p(1) kernel(uni) covs(GBA sexb2 fnac_anu empmasde50 primario)  
extraccion
local h1=0.5*bh
local h2=1.5*bh
...
rdrobust e`j' z, p(1) kernel(uni) h(`h1') covs(GBA sexb2 fnac_anu empmasde50 primario)  
extraccion
* IMPORTANTE: 
* El cambio de signo es porque el paquete asume que los de la derecha son los tratados. En nuestro caso es al revés.
...
rdrobust e`j' z, p(1) kernel(uni) h(`h2') covs(GBA sexb2 fnac_anu empmasde50 primario)   
extraccion
* IMPORTANTE: 
* El cambio de signo es porque el paquete asume que los de la derecha son los tratados. En nuestro caso es al revés.
...
export excel "$out/ResultadosLocalLinearCovs", sheetmodify cell(A`c')
restore
...
local c=`c'+4
 ...
 putexcel set "$out/ResultadosLocalLinearCovs.xls", modify
...
export excel "$out/ResultadosLocalLinearCovs", sheetmodify cell(A4)
restore
