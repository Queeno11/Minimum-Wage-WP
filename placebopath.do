...
gen zeroline = 0
	...
	local color2 0 0 255
		...
		legend(off)  ///
		title("") ///
		ytitle("Separation Rates 6 Months" "from Placebo Hike", margin(r=1) size(medsmall)) xtitle("Bandwidth (ARS)", size(medium)) ///
		graphregion(color(white)) plotregion(style(none)) 
		graph export "$out/PlaceboE6VsBandwidth.png", replace 
