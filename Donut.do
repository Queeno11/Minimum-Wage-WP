...
forvalue j=1/6 {
* Bandwidth óptimo: Resultados sin "hueco", referenciales
...
* Hueco de 5 pesos a cada lado

rdrobust e`j' z if z<-1 | z>1 , p(1) kernel(uni) // covs(sexb1 age ages province1-province24 secto2-secto14 tmemp1-tmemp3)  
extraccion
* IMPORTANTE: 
* El cambio de signo es porque el paquete asume que los de la derecha son los tratados. En nuestro caso es al revés.
...
* Hueco de 10 pesos
rdrobust e`j' z if z<-5 | z>5, p(1) kernel(uni) // covs(sexb1 age ages province1-province24 secto2-secto14 tmemp1-tmemp3)  
extraccion
* IMPORTANTE: 
* El cambio de signo es porque el paquete asume que los de la derecha son los tratados. En nuestro caso es al revés.
...
export excel "$out/Donut", sheetmodify cell(A`c')
restore
...
local c=`c'+4
 ...
 putexcel set "$out/Donut.xls", modify
 
 putexcel A1 = "Aumento/Donut"
 putexcel B1 = "Nulo"
 putexcel H1 = "1 Peso"
 putexcel N1 = "5 Pesos"
...
export excel "$out/Donut", sheetmodify cell(A4)
restore
