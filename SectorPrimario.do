...
rdrobust primario z, p(1) kernel(uni) level(90) h(`h')
extraccion
...
rdrobust primario z, p(1) kernel(uni) h(`h')
extraccion
...
gen zeroline = 0
	...
	local color2 0 0 255
		...
		legend(off)  ///
		title("") ///
		ytitle(Primary Sector , margin(r=1) size(medium)) xtitle("Bandwidth (ARS)", size(medium)) ///
		graphregion(color(white)) plotregion(style(none)) 
		graph export "$out/SectorVsBandwidth.png", replace 
