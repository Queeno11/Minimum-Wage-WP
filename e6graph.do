...
local j=1
forvalues h=10(10)600 /* Aca itero sobre bdw no covariates empmasde50 primario*/ { 
* Covariables

rdrobust e6 z, p(1) kernel(uni) level(90) h(`h')
extraccion
...
rdrobust e6 z, p(1) kernel(uni) h(`h')
extraccion
...
* CORRER ESTO DE NUEVO EL ytitle ESTABA MAL
gen zeroline = 0
	local color1 maroon
	local color2 0 0 255
		...
		legend(off)  ///
		title("") ///
		ytitle(Separation Rates 6 Months from Hike, margin(r=1) size(medium)) xtitle("Bandwidth (ARS)", size(medium)) ///
		graphregion(color(white)) plotregion(style(none)) 
		graph export "$out/E6VsBandwidth.png", replace 
